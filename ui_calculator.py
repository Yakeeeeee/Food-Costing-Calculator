...
class CalculatorFrame(ctk.CTkFrame):
    ...
    def _create_ingredient_checkbox(self, index: int, ingredient: Dict[str, str], global_index: int = None):
        ...
        # Checkbox
        var = ctk.BooleanVar()
        # If ingredient already in selected_ingredients, pre-check
        try:
            ing_name = ingredient.get("Ingredient Name", "")
            if any(ing.get("Ingredient Name", "") == ing_name for ing in self.selected_ingredients):
                var.set(True)
        except Exception:
            pass
        ...
        # Get grams needed (support multiple possible key names)
        grams_needed = (ingredient.get("Grams Needed in Recipe")
                        or ingredient.get("Grams Needed")
                        or ingredient.get("Grams Needed (Recipe)")
                        or ingredient.get("Grams", ""))
        grams_text = ""
        try:
            if grams_needed is not None and str(grams_needed).strip() != "":
                grams_text = f" ({float(str(grams_needed)):.0f} g)"
        except Exception:
            grams_text = f" ({grams_needed})"
        
        # Ingredient info: include grams needed
        ingredient_text = f"{ingredient.get('Ingredient Name', '')}{grams_text} - ${cost_str} per recipe"
