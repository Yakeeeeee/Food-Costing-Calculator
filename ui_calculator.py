...
class CalculatorFrame(ctk.CTkFrame):
    ...
    def _on_search_ingredients(self, event=None):
        ...
        for i, ingredient in enumerate(filtered_ingredients):
            self._create_ingredient_checkbox(i, ingredient)
    ...
    def _create_ingredient_checkbox(self, index: int, ingredient: Dict[str, str]):
        ...
        checkbox = ctk.CTkCheckBox(
            ...
            text="",
            variable=var,
            command=lambda idx=index, checked=var: self._on_ingredient_selection(idx, checked.get()),
            fg_color="#4cafef",
            hover_color="#3d8bc0"
        )
        ...
        # Ingredient info
        ingredient_text = f"{ingredient.get('Ingredient Name', '')} - ${ingredient.get('Cost per Recipe', '0.00')} per recipe"
        ...
        checkbox_frame.index = index
    
    def _on_ingredient_selection(self, index: int, selected: bool):
        """Handle ingredient selection/deselection"""
        if selected and index not in [ing["index"] for ing in self.selected_ingredients]:
            self.selected_ingredients.append({
                "index": index,
                "data": self.all_ingredients[index]
            })
        elif not selected:
            self.selected_ingredients = [ing for ing in self.selected_ingredients if ing["index"] != index]
    ...
    def _add_new_ingredient_to_recipe(self):
        ...
        self.selected_ingredients.append({
            "index": -1,  # -1 indicates new ingredient
